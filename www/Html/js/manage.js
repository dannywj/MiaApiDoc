    ...
    ajaxGetJson('Docs/Api', 'getAllList', {}, function (re) {
        ...
        for (var key in item_list) {
            ...
            for (var j = 0; j < item_list[key].length; j++) {
                ...
                row += '<tr><td style=""><a id="{4}" target="_blank"  href="api.html?id={0}"> {1}  {2}</a></td>{5}<td>{3}</td></tr>'.format(info.id, info.url, redMarkHtml(info.name) + redMarkVersion(info.create_version, info.update_version,info.current_version), btn, formatUrlKey(info.url), other_info);
            }
        }
        row += '</table>';
        $("#sp_result").html(row);
